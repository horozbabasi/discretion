...
### D63 - isComposing, answered on all three sites

**All three wait correctly.** An Enter pressed while an IME is composing
commits the candidate and does NOT submit, on ChatGPT, Claude and Gemini. The
adapters' unilateral skip of composing Enters matches what every site does, and
the last open send route is closed.

Measured 2026-09-03, attached to a browser the user launched and logged into by
hand (`navigator.webdriver = False`), with the composition generated by CDP
`Input.imeSetComposition` and the Enter by `Input.dispatchKeyEvent`:

| site | composer | composing Enter | control (plain Enter) |
| --- | --- | --- | --- |
| chatgpt | `#prompt-textarea` | `isComposing:true, trusted:true` - not sent | sent |
| claude | `[contenteditable][role=textbox]` | `isComposing:true, trusted:true` - not sent | sent |
| gemini | `.ql-editor[contenteditable]` | `isComposing:true, trusted:true` - not sent | sent |

Every Enter arrived `trusted: true`, which is what makes this an answer about
the sites' real handlers rather than about synthetic events they would ignore.

#### The login wall, and the route that was not an evasion

Playwright LAUNCHING the browser sets `--enable-automation`, `navigator.webdriver`
becomes true, and all three login challenges refuse it. That was not worked
around.

The route that worked: the user starts an ordinary browser with
`--remote-debugging-port` and a non-default `--user-data-dir`, logs in by hand,
and the probe attaches over CDP the way DevTools does. Nothing is spoofed, no
fingerprint forged, no check defeated. The measured difference is the whole
argument - `navigator.webdriver` is `False` because the browser genuinely is
not automated, not because anything claimed otherwise.

#### The control failed on two sites, and it was right to

The first run reported chatgpt `ok` and claude and gemini
`control failed, result not trustworthy`. The natural reading - that the
observation method was broken - was wrong, and so was assuming the sites simply
behaved differently.

**The composition was still open.** The probe pressed the composing Enter and
then, without closing the composition, pressed "a plain Enter" - which was not
plain at all. It was a second COMPOSING Enter, which commits rather than sends.
ChatGPT's handler happens to end the composition on the first Enter, so only
there did the control run as a control.

So the control was measuring nothing on two of three sites, and **it said so
rather than reporting a pass.** That is the failure direction a control is
supposed to have: this is the same mechanism that caught the logged-out landing
page at D60, failing safe again.

The fix is a control on the control. The composition is now cancelled
explicitly, the composer cleared, and the text typed WITHOUT an IME - and the
control Enter's own `isComposing` is recorded and printed:

```
control Enter: [{'isComposing': False, 'keyCode': 13, 'trusted': True}]
```

A control Enter that still arrives composing is now reported as
`control Enter was still composing` - inconclusive - rather than as a failed
send. The distinction matters: one says the site did not send, the other says
we never asked it to.

#### What this does and does not close

**Closed:** the `isComposing` route, on all three sites, with the answer that
the adapters' existing behaviour is correct. No code change was needed, which
is the outcome the measurement existed to establish rather than assume.

**Still open:** `form.submit()`, which fires no event any listener can see, and
a click on a control no adapter recognises - whose obvious fix would intercept
the STOP button. Both remain documented limitations (D57b), unchanged.
