...
**SIX ITEMS ARE OPEN AFTER M12. None may be reported as done.**

1. **The eight non-English catalogues are machine-generated and unreviewed.** Structurally validated by tests, semantically unverified by any native speaker. **A release blocker, and a store-submission blocker because listings are per-locale** (D53). M12 SCOPED IT: of 116 catalogue keys, **21 are safety-critical** - where a mistranslation causes a WRONG SAFETY DECISION rather than confusion (the two send controls, `keepOriginal`/`maskThis`, all four `panel.degraded.*`, `unwitnessed`, the three `popup.status.*` states, and Quick Redact's `mask`/`restore`/`unavailable`/`memoryOnly`). That is **254 words per locale**, a 20-30 minute read for a native speaker, not a translation contract. Recommended policy: a locale ships when a speaker has read its 21; a locale with no reviewer is DROPPED from the build rather than shipped unreviewed - `manifest.default_locale = 'en'` makes the fallback clean, and English is honest where a confidently wrong safety button is not.
2. **A send can still reach the site by three routes** (D57, D57a, D57b). Not primarily an i18n defect: all three adapters `return` silently when a click does not match their send selector, and `healthCheck` blocks nothing. MEASURED 2026-09-02, no locale-driven leak today. Of four routes found, one is CLOSED (`form.requestSubmit()`/native submission, by the new submit backstop, verified break-then-fix in a real browser); one is PERMANENTLY OPEN (`form.submit()` fires no event — no listener can see it, and closing it would need main-world script injection this extension refuses); and two stay open — a click on an unrecognised control, whose obvious fix would intercept the STOP button because "no send control resolves" is also every moment a response is streaming, and an Enter with `isComposing`, settled on our side and needing a signed-in IME session to settle on the site's. `scripts/probe-submit-routes.py` exercises all five routes in a real browser and is a regression gate.
