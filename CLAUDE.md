...
**SIX ITEMS ARE OPEN. None may be reported as done.** (Was six; the store-asset and privacy-policy blockers are closed. The translation item changed CHARACTER rather than closing - see 1.)
...
2. **A send can still reach the site by three routes** (D57, D57a, D57b). Not primarily an i18n defect: all three adapters `return` silently when a click does not match their send selector, and `healthCheck` blocks nothing. MEASURED 2026-09-02, no locale-driven leak today. Of four routes found, one is CLOSED (`form.requestSubmit()`/native submission, by the new submit backstop, verified break-then-fix in a real browser); one is PERMANENTLY OPEN (`form.submit()` fires no event — no listener can see it, and closing it would need main-world script injection this extension refuses); and two stay open — a click on an unrecognised control, whose obvious fix would intercept the STOP button because "no send control resolves" is also every moment a response is streaming, and an Enter with `isComposing`, settled on our side and STILL OPEN on the site's. M13 established that CDP `Input.imeSetComposition` produces a genuine TRUSTED composing Enter, so no human at an IME is needed - only a logged-in session. **Playwright LAUNCHING the browser trips the bot wall** (`navigator.webdriver` true) and that is NOT to be worked around - no stealth flags, no fingerprint spoofing. The route that works is `--attach`: the user starts an ordinary browser with `--remote-debugging-port` and a non-default `--user-data-dir`, logs in by hand, and the probe connects over CDP. Measured attached: `navigator.webdriver=False` and `Input.imeSetComposition` accepted. Nobody has run it yet, so all three sites are UNANSWERED (D61). `docs/manual-checks/isComposing.md` is the two-command handover. **The first probe reported WAITS CORRECTLY while testing the logged-out LANDING PAGE**, whose textarea has no send handler; its own `--control` caught it. Always run it with `--control` (D60). `scripts/probe-submit-routes.py` exercises all five routes in a real browser and is a regression gate.
