...
**Milestones are built strictly one at a time**, tests passing, before the next starts. **M1-M11 COMPLETE.** The extension protects, has a popup, an options page, nine locales, an accessibility audit, a submit backstop, a written README and a store-listing draft; a fresh clone builds and loads it. **M12 (library publication) NOT STARTED.** Verified 2026-09-02: **1,271 tests / 81 files**, typecheck (source AND test files), lint and build all clean, each confirmed by its own exit code.

**FOUR ITEMS ARE OPEN AFTER M11. None may be reported as done.**
