...
**SIX ITEMS ARE OPEN. None may be reported as done.** Closed since M12: the `isComposing` send route (D63); the store promotional assets and review-panel screenshot (D60/D62); and the private-repository blocker - the repo is PUBLIC as of 2026-09-03 and every URL the listing depends on was re-fetched anonymously at HTTP 200, including the store-required PRIVACY.md (D62, D64).

1. **Eight locales are unreviewed and therefore NOT SHIPPED.** No longer "a blocker to remember" - the build ENFORCES it: `reviewStateOf()` drops any locale without a speaker's sign-off, `REVIEW_SIGNOFFS` is empty, and `_locales/en` ships alone. A Turkish user gets an English UI via `default_locale`, which is honest where a confidently mistranslated "Mask this" is not. **A sign-off is a claim that a HUMAN who reads the language checked the 21 safety-critical strings; it must never be added on the strength of a machine translation, a model's opinion of its own output, or a round-trip back to English.** Sheets are ready in `docs/translation-review/` (21 keys, ~254 words each). Sign-offs carry a DIGEST, so rewording a covered string invalidates the sign-off and drops the locale again. Consequence to remember: the store listing must be ENGLISH-ONLY at first submission (D60). **DECIDED at D61: launch English-only.** An AI-assisted pass over the 21 strings was run as a DEFECT-FINDING pass and explicitly NOT as a sign-off - I authored the translations, so a systematic error is invisible to my own check, and the absence of findings in a language is not evidence of correctness. It found three things (tr protected/unprotected differ by one letter; ar mixes verbal and adjectival for one binary status; fr `maskThis` drops the deictic) in `docs/translation-review/FINDINGS-ai-pass.md`. **None were fixed** - rewriting a translation without a reviewer is just more machine translation.
2. **A send can still reach the site by TWO routes** (D57, D57a, D57b, D63 - was three; isComposing is closed). Not primarily an i18n defect: all three adapters `return` silently when a click does not match their send selector, and `healthCheck` blocks nothing. MEASURED 2026-09-02, no locale-driven leak today. Of four routes found, one is CLOSED (`form.requestSubmit()`/native submission, by the new submit backstop, verified break-then-fix in a real browser); one is PERMANENTLY OPEN (`form.submit()` fires no event — no listener can see it, and closing it would need main-world script injection this extension refuses); and two stay open — a click on an unrecognised control, whose obvious fix would intercept the STOP button because "no send control resolves" is also every moment a response is streaming, and an Enter with `isComposing` - **that third one is now CLOSED**. Measured 2026-09-03 against real signed-in sessions on all three sites: a composing Enter commits the candidate and does NOT submit, so the adapters' skip is correct and no code change was needed (D63). The route: the user starts a browser with `--remote-debugging-port` and a non-default `--user-data-dir`, logs in by hand, and `probe-ime-live.py --attach 9222 --control` connects over CDP - `navigator.webdriver` False, nothing spoofed. **Always pass `--control`**, and note the control now verifies its OWN Enter is non-composing: without that it silently measured nothing on two of three sites, because the composition was still open and the 'plain' Enter was a second composing one. `scripts/probe-submit-routes.py` exercises all five routes in a real browser and is a regression gate.
...
6. **The product is named Discretion** (D64), decided and executed 2026-09-03: npm scope `@discretion`, repo `horozbabasi/discretion`, extension `Discretion`. **TRADEMARK IS NOT CLEARED, and this must not be edited into a clean bill of health** (D65): no register was queried - web search cannot reach USPTO TESS or EUIPO, and a search engine's silence is not a register's answer. Search did find **DiscreetTab**, a privacy browser extension; the marks share a root and both are browser extensions in the privacy category, which is precisely the similarity-plus-relatedness test. The user decided to proceed on that basis, as informed risk. Renaming again before the first npm publish is free; after it, npm has no rename, only deprecate-and-republish. SPEC records that "Discretion" is still an open pre-public question, and the packages are `@discretion/core` / `@discretion/data` (both verified unregistered). Renaming an UNPUBLISHED package is free; npm has no rename afterwards, only deprecate-and-republish. **Settle the product name BEFORE the first publish, not before the first release** (D59).
