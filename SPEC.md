...
M8  Stage 4 fusion, calibration, explanations, sensitivity profiles. Publish the calibration curve. Exposure score engine in core, its severity-weight data file with documented rationale, tests including the monotonicity property, and the playground exposure panel. Extend BENCHMARKS.md. [DONE. Held-out calibration ECE 2.63% vs 12.33% raw, fitted and evaluated on splits proved disjoint. Overlap resolution took GENERIC_SECRET 2.0%->100% and POSTAL_CODE 23.5%->100% precision by REASSIGNMENT, not elimination; total false positives 2,991->246. Stage 0 digit folding landed first and took recall in six native-digit languages from 66.17% to 99.75%. Open, carried to M9: GENERIC_SECRET recall 55.4% (a detection gap, not an overlap one), TAX_ID recall 91.2%, one over-confident calibration bucket, and the p50 latency budget missed under favourable conditions on both axes.]
