# Translation review — Japanese (`ja`)
...
> **Could someone reading only the Japanese text act in a way they did not
...
| key | English | Japanese | If this is wrong | Verdict | Suggested replacement |
| --- | --- | --- | --- | --- | --- |
| `panel.action.cancel` | Cancel | **キャンセル** | If this reads as "send", the user sends unmasked text while trying to stop. | | |
| `panel.action.maskAndSend` | Mask and send | **マスクして送信** | The button that sends. If it reads as "cancel", the user sends when they meant to stop. | | |
...
| `quick.memoryOnly` | The mapping between your text and its replacements is kept in memory only, and is erased when this popup closes. | **テキストと置換値の対応はメモリ上にのみ保持され、このウィンドウを閉じると消去されます。** | A privacy claim about where the text goes. Must not overstate or understate it. | | |
